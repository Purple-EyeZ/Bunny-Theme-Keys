...
function addSearchButtonEvent() {
    ...
    searchButton.addEventListener('click', () => {
        ...
        if (isHidden) {
            ...
            clearButtonFixed.style.display = fixedSearchBar.value.length > 0 ? 'block' : 'none';
        } else {
...
function toggleClearButton(inputElement, value) {
    const clearButton = elements[inputElement.id === 'search-bar' ? 'clearButton' : 'clearButtonFixed'];
    const shouldShow = value.length > 0 &&
        (inputElement.id !== 'fixedSearchBar' || inputElement.classList.contains('show'));
...
function calculateScore(text, query) {
    ...
    queryWords.forEach(queryWord => {
        ...
        const exactMatchIndex = words.findIndex(w => w === queryWord);
...
function sortBlocks(container) {
    ...
    blocks.sort((a, b) => {
        const titleA = a.querySelector('h3').textContent.replace(/(\d+)/, (_, num) => String(num).padStart(10, '0'));
        const titleB = b.querySelector('h3').textContent.replace(/(\d+)/, (_, num) => String(num).padStart(10, '0'));
...
function filterBlocks(query) {
    ...
    otherBlocks.forEach(block => block.style.display = 'none');

    if (!query) {
        blocks.forEach(block => {
            block.style.display = 'flex';
            block.querySelector('h3').innerHTML = block.dataset.originalTitle || block.querySelector('h3').textContent;
            block.querySelector('.description').innerHTML = block.dataset.originalDescription || block.querySelector('.description').textContent;
        });
    ...
    blocks.forEach(block => processBlock(block, query, results.active));
    otherBlocks.forEach(block => processBlock(block, query, results.other));
    ...
    // Update subtext
    const subtextMessage = results.other.length > 0
...
function processBlock(block, query, results) {
    if (!block.dataset.originalTitle) {
        ...
        block.dataset.originalDescription = block.querySelector('.description').textContent;
    }

    const title = block.dataset.originalTitle;
    const description = block.dataset.originalDescription;

    const titleScore = calculateScore(title, query) * CONSTANTS.SCORE.TITLE_MULTIPLIER;
    const descriptionScore = calculateScore(description, query);
    const totalScore = titleScore + descriptionScore;

    if (totalScore > 0) {
        block.querySelector('h3').innerHTML = highlightMatch(block.dataset.originalTitle, query);
        block.querySelector('.description').innerHTML = highlightMatch(block.dataset.originalDescription, query);
...
function resetBlocksDisplay(category) {
    ...
    blocks.forEach(block => {
        ...
        if (block.dataset.originalTitle) {
            ...
            block.querySelector('.description').innerHTML = block.dataset.originalDescription;
        }
    });
...
export const initSearch = () => {
    const handleCategoryChange = (event) => {
        ...
        if (searchState.wasCleared || !searchState.currentValue) {
            ...
            const subtextMessage = document.getElementById('searchSubtext').textContent;
            updateSearchSubtext(subtextMessage);
        }
    };
