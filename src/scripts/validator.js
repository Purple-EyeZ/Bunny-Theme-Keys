...
function isValidHexColor(value) {
    return /^#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{8}|[0-9A-Fa-f]{3}|[0-9A-Fa-f]{4})$/.test(value);
}
...
function isValidRgbaColor(value, key = null, errors = null) {
    ...
    const isValid = r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255 && a >= 0 && a <= 1;
...
function isValidColor(value) {
    return typeof value === 'string' && (isValidHexColor(value) || isValidRgbaColor(value) || value === "transparent");
}
...
function validateRequired(obj, field, type, errors) {
    if (typeof obj[field] !== type || (type === 'string' && obj[field].trim() === '')) {
...
function validateFonts(fontObject, errors, warnings = false) {
    ...
    REQUIRED_FONTS.forEach(font => {
        if (!fontObject[font]) {
            if (font === "ggmono-Normal" && fontObject["SourceCodePro-Semibold"]) {
                warnings.push('"SourceCodePro-Semibold" has been replaced by "ggmono-Normal" in recent Discord versions. Consider updating your font pack.');
                return;
            }
    ...
    if (urls.size === 1 && Object.keys(fontObject).length > 1) {
        errors.push('All fonts cannot use the same URL. Different font styles must point to different files.');
    } else if (urls.size < Object.keys(fontObject).length * 0.5) { // if less than 50% of the fonts are unique
        warnings.push('Most font styles use the same file, which can prevent visual differences and cause display inconsistencies. Use distinct files for each style.');
    }
...
document.getElementById('validateButton').addEventListener('click', function () {
    ...
    try {
        ...
        if (errors.length > 0) {
            ...
            const successMessage = type === 'fontpack' ? 'Valid font pack!' : 'Valid theme!';
...
function validateJSON(jsonObject, _lines) {
    ...
    if (!isFontPack && !isTheme) {
        errors.push('Invalid JSON: must contain either "main" for font packs or "semanticColors"/"rawColors" for themes.');
        return { errors, warnings, type: 'unknown' };
    }

    if (isFontPack && isTheme) {
        errors.push('Invalid JSON: cannot contain both font pack and theme properties.');
        return { errors, warnings, type: 'unknown' };
    }
...
function validateFontPack(jsonObject, errors, warnings) {
    ...
    const urls = validateFonts(jsonObject.main, errors, warnings);
...
function validateTheme(jsonObject, errors, warnings) {
        ...
        if (!validateObject(jsonObject, 'background', errors)) return;
            ...
            ['blur', 'alpha'].forEach(prop => {
                if (jsonObject.background[prop] !== undefined) {
                    ...
                    } else if (jsonObject.background[prop] < 0 || jsonObject.background[prop] > 1) {
    ...
    if (jsonObject.fonts) {
        if (!validateObject(jsonObject, 'fonts', errors)) return;
            ...
            const fontUrls = validateFonts(jsonObject.fonts, errors, warnings, true);
        }
    }
        ...
        if (jsonObject.plus?.icons) {
            if (!validateObject(jsonObject.plus, 'icons', errors)) return;
                ...
                for (const [iconName, iconValue] of Object.entries(jsonObject.plus.icons)) {
                    ...
                    } else if (typeof iconValue === 'string' && !isValidHexColor(iconValue)) {
                        errors.push(`The value of "${iconName}" must be a valid HEX color string.`);
                    } else if (typeof iconValue !== 'string' && !Array.isArray(iconValue)) {
            ...
            if (jsonObject.plus.mentionLineColor && !isValidHexColor(jsonObject.plus.mentionLineColor)) {
